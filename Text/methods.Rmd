...
The $z$ parameter has an important function in the model. In particular, $z$ determines the level $\tau_z$, if a patient survives that moment, the patient is considered cured of that disease. Previously $z \sim Po^+ (\mu)$ and in order to make later inferences about $\mu$, a hierarchical model is taken assigning a hyper prior distribution for $\mu$. If we take $\mu \sim Ga(a_0, b_0)$, then the subsequent conditional distribution of $\mu$ only depends on the value of $z$.
...
In the implementation we found a failure because the acceptance rate is desirable to be between 20\% and 40\% [@robert2010introducing] and the user did not have the possibility to manipulate the parameters of the auxiliary distribution proposed in the article that directly affect the acceptance rate. We note that increasing the coefficient of variation of the proposed distribution will decrease the acceptance rate and it will take longer for the algorithm to characterize the desired distribution, if it decreases the coefficient of variation it will increase the acceptance rate, in an extreme case it will cost the algorithm characterize the distribution in your domain.
...
In both the standard model and the proportional hazards rates with cure rate, the covariates can be incorporated by modeling $\pi$ or $\theta$ as a function of the covariates. For the mix model, @Kuk1992 modeled $\pi$ with a logistic regression that has the drawback of not having proportional hazard rates built for the entire population. For the model with proportional risk rates with cure rate, @Kuk1992 proposed that the covariates be incorporated into the model through the parameter $\theta$ to obtain proportional risk rates for the entire population.

To conduct an analysis with covariates, @Nieto-Barajas2008 proposes a semi-parametric model that is similar to the @Cox1972 proportional hazards model. The base risk is modeled by \eqref{apriori} to explicitly impose the cure fraction and the cure dimension. Additionally, the model has flexibility so that each individual can have a different dependent cure time of $z_i$. For the individual $i$ with a vector of covariates of size $p$ that possibly depends on time, $\symbfit{x}_i(t)$, they propose that the risk function be of the form

\begin{align}\label{nieto}
h_i(t\mid\symbfit{x}_i,z_i)=h(t\mid z_i)e^{\theta'\symbfit{x}_i(t)},
\end{align}

where $\theta$ is a vector of regression parameters and $h(t \mid z_i)$ is the base risk function. @Nieto-Barajas2008 assigns $h(t \mid z_i)$ a nonparametric mixture as initial distribution that is capable of modeling the cure fraction and the cure time
...
The \eqref{grandson} model is not a proportional risk rate model because there are different cure times indexed by $z_i$. This characteristic allows to estimate the cure time for each individual by assigning a previous function $f(z_i)$ dependent on the covariates.
...
You can obtain the complete conditional posterior distributions that are necessary for the implementation of a Gibbs sampler. The complete conditional distributions for $\lambda_k$ and $u_k$ are given in a similar way to the nonparametric case with no covariates.
...
As in the case without covariates, the cure time $\tau_{z_i}$ can be estimated by $\tau_{\hat{z}_i}$, where $\hat{z}_i$ is the median for being discrete in nature or, if conservative, a higher quantile of the posterior distribution of $z_i$.
